...
saved only summary metrics, not per-row predictions. This script:
  ...
  1. REPRODUCES that run bit-for-bit (same seed, same grids, same
     validation-selection rule, same scaler procedure, same row set),
  2. RECONCILES the reproduced test metrics against the frozen
     outputs/ablation_results.csv and ABORTS if they do not match,
  3. only then runs DeLong AUC-difference tests and McNemar tests on the
     recovered predictions, and writes the Chapter 4 tables.

This is a reproduction of the single frozen test evaluation, NOT a new
evaluation. No model choice, hyperparameter, or feature is altered.
The reconciliation gate (step 2) is what entitles the report to say the
statistical tests were computed on the frozen predictions.

RUN (from the pipeline root, venv active):
    python scripts\delong_tests.py data\config3_features.csv

OPTIONS:
    --single-thread    refit XGBoost with n_jobs=1. Use only if the
                       default run fails reconciliation (threading
...
References:
    ...
    Sun & Xu (2014), IEEE Signal Processing Letters 21(11)  [fast algorithm]
    McNemar (1947), Psychometrika 12(2). Edwards (1948) continuity correction.
"""
